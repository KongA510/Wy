...
ht["state"] = "Released";
var result = item.apply(ht); // 等价于 setProperty 两次 + apply()
```
...
item.setProperty("item_number", "P-001");
var result = item.apply("get");  // 设置 action = "get" 并发送
```
...
var updateResult = updateItem.apply("edit", props);
```
