...
## 第一阶段：一次性获取完整 WBS 与 Activity2 数据

### 2.1 核心 AML 调用：GetItemRepeatConfig
<p>
...
```csharp
public static List<TaskNode> BuildTaskTreeFromXml(string wbsXml)
{
...
```csharp
private static void SortRelationships(System.Xml.XmlNode parentNode)
{
...
```csharp
private static TaskNode BuildWbsNode(
...
```csharp
private static TaskNode BuildActivityNode(
...
```csharp
public enum TaskType { TASK = 0, MILESTONE = 1, SUMMARY = 2 }
