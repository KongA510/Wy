...
export const menuData: MenuNode[] = [
  ...
  { id: 'server-api', label: 'Aras 服务端文档', icon: 'Platform', path: '/server-api', description: 'Aras Innovator .NET API 参考，覆盖 IOM、OAuth、IOME、Aras.Net 命名空间' },
...
function buildServerApiSearchIndex(nodes: ArasDocNode[], parentLabel = '') {
  for (const node of nodes) {
    const category = parentLabel ? `${parentLabel} > ${node.name}` : node.name
    if (node.type === 'item' && node.path) {
      searchableItems.push({ id: 'srvapi-' + node.id, title: node.name, category, path: node.path, description: `Aras 服务端文档 - ${category}` })
    }
