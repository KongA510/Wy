<template>
  <div class="integ-doc-view">
    <div class="integ-doc-body">
      <div class="integ-doc-content">
        ...
        <div v-else class="not-found">
          ...
          <h2>文档未找到</h2>
          <p>该接口文档尚未创建，请在 IntegrationDocView 中注册对应组件</p>
    ...
    <aside v-if="comp && tocItems.length" class="integ-doc-toc">
...
import XjOverviewView from './integration/XjOverviewView.vue'
import XjGetClassStructureView from './integration/XjGetClassStructureView.vue'
import XjGetBodyPartCodeView from './integration/XjGetBodyPartCodeView.vue'
import XjBomInterfaceView from './integration/XjBomInterfaceView.vue'
import XjBomInterfaceServerView from './integration/XjBomInterfaceServerView.vue'

const route = useRoute()

const docComponents: Record<string, any> = {
  'xj-overview': XjOverviewView,
  'xj-get-class-structure': XjGetClassStructureView,
  'xj-get-body-part-code': XjGetBodyPartCodeView,
  'xj-bom-interface': XjBomInterfaceView,
  'xj-bom-interface-server': XjBomInterfaceServerView
}
...
<style scoped>
.integ-doc-view { width: 100%; }
.integ-doc-body { display: flex; gap: 32px; align-items: flex-start; }
.integ-doc-content { flex: 1; min-width: 0; max-width: 800px; }
.integ-doc-toc { position: fixed; right: max(32px, calc((100vw - 1200px) / 2)); top: 140px; width: 220px; max-height: calc(100vh - 180px); overflow-y: auto; padding: 0 12px 0 16px; border-left: 2px solid #e5e7eb; z-index: 40; }
.integ-doc-toc::-webkit-scrollbar { width: 3px; }
.integ-doc-toc::-webkit-scrollbar-thumb { background: #d1d5db; border-radius: 3px; }
.toc-title { font-size: 14px; font-weight: 700; color: #111827; margin-bottom: 12px; letter-spacing: 0.5px; }
.toc-list { list-style: none; margin: 0; padding: 0; }
.toc-item { margin: 0; }
.toc-link { display: block; font-size: 13px; line-height: 1.6; color: #6b7280; text-decoration: none; padding: 3px 0 3px 12px; border-left: 2px solid transparent; margin-left: -18px; transition: color 0.15s, border-color 0.15s; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
@media (max-width: 1100px) { .integ-doc-toc { display: none; } }
</style>
