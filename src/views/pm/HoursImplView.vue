<template>
  <article class="doc-content">
    ...
    <pre><code class="language-typescript">{{ clientCode }}</code></pre>
    ...
    <pre><code class="language-typescript">{{ arasBodyCode }}</code></pre>
    <h3>2.4 方法注册表（methods.ts）</h3>
    <p>所有后端 Method 名称集中管理，前端通过常量引用，避免硬编码字符串散落在各模块。</p>
    <pre><code class="language-typescript">{{ methodsCode }}</code></pre>
    ...
    <pre><code class="language-typescript">{{ wireViewCode }}</code></pre>
    <h2>四、后端核心 Method 实现</h2>
    <p>所有后端 Method 共享统一的权限提升模式：</p>
    <pre><code class="language-csharp">{{ permPattern }}</code></pre>
    ...
    <pre><code class="language-csharp">{{ getProjectListCode }}</code></pre>
    ...
    <pre><code class="language-csharp">{{ saveProjectDataCode }}</code></pre>
    ...
    <pre><code class="language-csharp">{{ updateProjectDataCode }}</code></pre>
    ...
    <pre><code class="language-csharp">{{ projectDetailCode }}</code></pre>
    ...
    <pre><code class="language-csharp">{{ getHourDataCode }}</code></pre>
    ...
    <pre><code class="language-csharp">{{ hoursDetailCode }}</code></pre>
    ...
    <pre><code class="language-typescript">{{ exportExcelCode }}</code></pre>
    ...
    <h2>五、状态机</h2>
    <p>工时记录的审批状态流转如下：</p>
    <pre><code class="language-text">{{ stateMachine }}</code></pre>
    <ul>
    ...
    <pre><code class="language-typescript">{{ routerCode }}</code></pre>
    <h2>七、总结</h2>
    <table>
...
const projectTree = `Kong_A_Hours/
├── src/
│   ├── api/
...
const clientCode = `// api/client.ts — 统一调用入口
...
const arasBodyCode = `// api/arasBody.ts — XML Body 构造器
...
const methodsCode = `// api/methods.ts — Aras Method 注册表
...
const wireViewCode = `// api/modules/timesheet.ts — Wire↔View 映射
...
const permPattern = `// 所有后端 Method 共享的权限提升模式
...
const getProjectListCode = `// IC_getProjectList — 按用户 Identity 筛选项目
...
const saveProjectDataCode = `// IC_saveProjectData — 提交新工时
...
const updateProjectDataCode = `// IC_updateProjectData — 修改工时
var projectData = Newtonsoft.Json.JsonConvert
    .DeserializeObject<HoursProject>(
        this.getProperty("projectData", ""));
...
const reviewCode = `// IC_allReviewPassOrReject — 批量审核
...
const reportCode = `// IC_getReportData — 年度统计
...
const detailsCode = `// IC_Hours_Details — AML 按年月查询明细
...
const projectDetailCode = `// IC_getProjectDetailData — SQL 多维度 JOIN
...
const getHourDataCode = `// getProjectHourData — 按用户 + 周范围查询
var created_by_id = inn.getUserID();
...
const routerCode = `// router/index.ts — Hash 路由（Aras IFRAME 兼容）
...
const hoursDetailCode = `// IC_Hours_Details — AML 按年月 + 项目查询工时明细
var year = Convert.ToInt32(this.getProperty("year", "1999"));
var month = Convert.ToInt32(this.getProperty("month", "4"));
var project_id = this.getProperty("project_id", "");
var startData = new DateTime(year, month, 1);
...
const exportExcelCode = `// utils/exportExcel.ts — 导出 Excel 文件
