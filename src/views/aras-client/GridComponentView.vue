<template>
  <article class="doc-content">
    ...
    <h3>3.1 构造与渲染</h3>
    <pre v-pre><code class="language-javascript">const grid = new Grid(domElement, {
...
// 启动渲染
await grid.render();</code></pre>

    <h3>3.2 列数据管理（head）</h3>
    <pre v-pre><code class="language-javascript">// head — HeadWrap 对象
grid.head = new Map([
...
// 删除列
grid.head.delete('col_id');</code></pre>

    <h3>3.3 行数据管理（rows）</h3>
    <pre v-pre><code class="language-javascript">// rows — RowsWrap 对象
grid.rows = new Map([
  ['row_1', {
    col_name: '螺栓 M8x30',
    col_state: 'Released',
...
// 删除行
grid.rows.delete('row_1');</code></pre>

    <h3>3.4 设置管理（settings）</h3>
    <pre v-pre><code class="language-javascript">// 冻结前两列
...
grid.settings.focusedCell = {
  ...
  editing: false
    ...
    <h3>3.5 退出编辑模式</h3>
    <pre v-pre><code class="language-javascript">// 强制退出编辑模式
grid.cancelEdit();</code></pre>

    <h2>四、完整应用示例</h2>
    <pre v-pre><code class="language-javascript">// BOM 表格完整示例
