<template>
  <article class="doc-content">
    ...
    <blockquote>
      ...
      <p><strong>替代方案：</strong>新项目中应使用 <code>&lt;aras-toolbar&gt;</code> Web Component（参见 <router-link to="/aras-client/toolbar-component">Toolbar 工具栏组件</router-link>）。</p>
    </blockquote>
    ...
    <p>创建 Toolbar 实例。</p>
    <h4>语法</h4>
    <pre v-pre><code class="language-javascript">var toolbar = new Aras.Client.Controls.Public.Toolbar();</code></pre>
    <h4>参数</h4>
    <p>无。</p>
    <h4>返回值</h4>
    <p>无（构造函数）。</p>
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">// 通过 clientControlsFactory 订阅点击事件
clientControlsFactory.on(toolbar, "onClick", function (toolbarItem) {
  ...
  if (itemId === "btn_save") {
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">clientControlsFactory.on(toolbar, "onChange", function (toolbarItem) {
  ...
  console.log("下拉框选中值变更:", selectedValue);
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">clientControlsFactory.on(toolbar, "onKeyDown", function (toolbarItem) {
  ...
  console.log("当前输入内容:", textValue);
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">clientControlsFactory.on(toolbar, "onDropDownItemClick", function (text) {
  ...
  filterGridItems(text);
    ...
    <p>无。</p>
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">var toolbar = new Aras.Client.Controls.Public.Toolbar();
// 从客户端资源文件加载工具栏 XML
toolbar.loadXml("../ClientSolutions/Toolbars/MyToolbar.xml");</code></pre>
    ...
    <p>无。</p>
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">var xmlStr = '&lt;toolbar&gt;' +
  '&lt;button id="btn_new" label="新建" image="../images/new.svg" /&gt;' +
  '&lt;separator /&gt;' +
  '&lt;button id="btn_save" label="保存" image="../images/save.svg" /&gt;' +
  '&lt;/toolbar&gt;';
toolbar.loadToolbarFromStr(xmlStr);</code></pre>
    ...
    <p>无。</p>
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">toolbar.loadXml("../ClientSolutions/Toolbars/MyToolbar.xml");
toolbar.show();  // 加载后显示</code></pre>
    ...
    <p>无。</p>
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">// 加载多个工具栏配置后，按需切换
toolbar.showToolbar("main_toolbar");       // 显示主导航工具栏
toolbar.showToolbar("context_toolbar");    // 切换为上下文工具栏</code></pre>
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">var activeBar = toolbar.getActiveToolbar();
console.log("当前工具栏ID:", activeBar.getId());</code></pre>
    ...
    <p>无。</p>
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">// 在数据加载期间禁用工具栏，防止用户误操作
toolbar.disable();
fetchDataFromServer().then(function () {
  toolbar.enable();
    ...
    <p>无。</p>
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">// 紧凑模式：隐藏按钮标签只显示图标
toolbar.showLabels(false);

// 完整模式：显示图标+文字标签
toolbar.showLabels(true);</code></pre>
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">var toolbarId = toolbar.getId();
console.log("当前工具栏 ID:", toolbarId);</code></pre>
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">var saveBtn = toolbar.getItem("btn_save");
if (saveBtn) {
  // 动态修改按钮属性
  ...
  saveBtn.setLabel("保存中...");
    ...
    <p>无。</p>
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">// 根据权限显示不同按钮
if (currentUser.canDelete) {
  ...
  toolbar.hideItem("btn_delete");
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">// 在按钮旁弹出菜单时使用
var btnPos = toolbar.getButtonXY("btn_actions");
console.log("按钮位置:", btnPos);
// 可用于定位自定义弹出层
showPopupAt(btnPos);</code></pre>
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">var btnId = toolbar.getButtonId("保存");
console.log("'保存' 按钮的 ID:", btnId);  // 输出类似 "btn_save"</code></pre>
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">if (toolbar.isButtonEnabled("btn_save")) {
  // 保存按钮可用时才执行
  toolbar.getItem("btn_save").performClick();
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">var label = toolbar.getButtonLabelById("btn_save");
console.log("btn_save 的标签:", label);  // 输出 "保存"</code></pre>
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">if (!toolbar.isButtonVisible("btn_advanced")) {
  console.warn("'高级' 按钮因工具栏溢出而被隐藏，建议使用常用功能的快捷按钮");
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">var btns = toolbar.getButtons(", ");
console.log("工具栏包含:", btns);
// 输出类似: "btn_new, btn_save, sep_1, btn_delete, btn_refresh"</code></pre>
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">var size = toolbar.getItemSize("btn_save");
// size 类似 "32,32"
var dimensions = size.split(",");
var width = parseInt(dimensions[0]);
var height = parseInt(dimensions[1]);
console.log("按钮尺寸:", width + "x" + height);</code></pre>
    ...
    <!-- ==================== 综合示例 ==================== -->
    <h2>综合使用示例</h2>
    <pre v-pre><code class="language-javascript">// 完整的工具栏初始化和事件绑定示例
