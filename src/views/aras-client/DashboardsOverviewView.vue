<template>
  <article class="doc-content">
    ...
    <h2>一、仪表板架构</h2>
    <pre v-pre><code class="language-text">仪表板 (Dashboard)
  ...
  └── Widget 3 (Report Widget)
        ...
        ├── 渲染: 自定义内容
        └── widget_data: { item_id, report_config }</code></pre>
    ...
    <pre v-pre><code class="language-javascript">// Grid Widget 的 widget_data 示例
{
  ...
  "max_results": "100"
    ...
    <h2>四、Widget 生命周期</h2>
    <pre v-pre><code class="language-text">Widget 初始化（第一次）
  → WidgetLayout 创建
  → widget_data 解析
...
Widget 更新（widget_data 属性变化）
  ...
  └─ 已改变 → Widget 销毁 → 重新初始化 WidgetLayout → 全新控件实例</code></pre>
