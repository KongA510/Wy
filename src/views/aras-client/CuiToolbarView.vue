<template>
  <article class="doc-content">
    ...
    <h2>二、公共 API</h2>
    <pre v-pre><code class="language-javascript">// 模块导出单一函数
function cuiToolbar(
  control: Toolbar,          // Toolbar Web Component 实例
  location: String,          // CUI Location 名称
  options: Object            // 额外配置
): Promise<{destroy: Function, updateOptions: Function}></code></pre>
    ...
    <pre v-pre><code class="language-text">CUI 数据层次：
SearchViewLayout (CUI Layout)
  └── CommandBarSection: "Search Commands Bar"
        ...
        └── CommandBarItem: "More" (下拉菜单)
              type: "dropdown"
              children: ["Export", "Import", "Settings"]</code></pre>

    <h2>五、完整使用示例</h2>

    <h3>5.1 在 CUI Layout 中注册工具栏</h3>
    <pre v-pre><code class="language-javascript">// CUI 数据定义（服务端返回）
...
const initToolbarControl = (
  ...
  options
    ...
    <pre v-pre><code class="language-javascript">import { cuiToolbar } from './cui/cuiToolbar';
...
cuiToolbar(toolbarEl, 'Item_View_Commands', {
  ...
  updateOptions({
    ...
    <h3>5.3 工具栏按钮事件定义</h3>
    <pre v-pre><code class="language-javascript">// CommandBarItem 的事件处理
