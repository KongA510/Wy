<template>
  <article class="doc-content">
    ...
    <p>无返回值信息。</p>
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">// 一般不直接构造，而是通过 Toolbar 获取
const toolbar = top.aras.getMainWindow().document.getElementById('toolbar');
const toolbarItem = toolbar.getItem('myButton');</code></pre>
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">const toolbar = top.aras.getMainWindow().document.getElementById('toolbar');
const item = toolbar.getItem('saveButton');
const id = item.getId();  // 返回 'saveButton'</code></pre>
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">const item = toolbar.getItem('toggleButton');
if (item.getState()) {
  console.log('按钮处于按下状态');
    ...
    <p>无返回值。</p>
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">// 将按钮设为按下状态
item.setState(true);

// 取消按钮的按下状态
item.setState(false);</code></pre>
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">const item = toolbar.getItem('saveButton');
if (item.getEnabled()) {
  console.log('按钮已启用');
}</code></pre>
...
item.setEnabled(true);</code></pre>
    ...
    <p>无返回值。</p>
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">// 启用按钮
item.enable();</code></pre>
    ...
    <p>无返回值。</p>
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">// 禁用按钮
item.disable();</code></pre>
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">const textItem = toolbar.getItem('searchBox');
const currentText = textItem.getText();  // 获取搜索框中的文本</code></pre>
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">const textItem = toolbar.getItem('searchBox');
textItem.setText('请输入关键词');  // 设置搜索框提示文本</code></pre>
    ...
    <p>无返回值。</p>
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">const item = toolbar.getItem('numberInput');
item.setLabel('数量：');  // 在输入框前显示"数量："标签</code></pre>
...
console.log('位置:', bounds.x, bounds.y);</code></pre>
    ...
    <p>无返回值。</p>
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">const comboBox = toolbar.getItem('typeComboBox');
// 清空已有选项
comboBox.removeAll();
// 添加新选项
comboBox.add('type_part', '零件');
comboBox.add('type_doc', '文档');
comboBox.add('type_bom', 'BOM');</code></pre>
    ...
    <p>无返回值。</p>
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">const comboBox = toolbar.getItem('menuComboBox');
comboBox.removeAll();
comboBox.add('action_new', '新建');
comboBox.add('action_open', '打开');
comboBox.addSeparator();  // 插入分隔符
comboBox.add('action_exit', '退出');</code></pre>
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">const comboBox = toolbar.getItem('typeComboBox');
comboBox.remove('type_bom');  // 移除 BOM 选项</code></pre>
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">const comboBox = toolbar.getItem('typeComboBox');
comboBox.removeAll();  // 清空所有选项</code></pre>
...
console.log('共有 ' + count + ' 个选项');</code></pre>
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">const comboBox = toolbar.getItem('typeComboBox');
const itemValue = comboBox.getItem('type_part');  // 获取指定选项的值</code></pre>
...
console.log('当前选中: ' + selected);</code></pre>
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">const comboBox = toolbar.getItem('typeComboBox');
comboBox.setSelected('type_doc');  // 选中"文档"选项</code></pre>

    <!-- ==================== 综合示例 ==================== -->
    <h3>综合示例</h3>
    <p>以下示例演示了操作工具栏中 ComboBox 类型的 ToolbarItem 的典型流程：</p>
    <pre v-pre><code class="language-javascript">// 获取 Toolbar 及其中的 ComboBox 项目
const toolbar = top.aras.getMainWindow().document.getElementById('toolbar');
