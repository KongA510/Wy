    ...
    <pre v-pre><code class="language-javascript">// Validate 事件处理器必须返回以下结构：
{
  ...
  validationMessage: String   // 错误提示信息（可选）
    ...
    <h3>3.2 实战示例</h3>
    <pre v-pre><code class="language-javascript">// 示例1：零件号格式验证
...
function validateDueDate(eventArgs) {
  ...
  return { valid: true };
    ...
    <pre v-pre><code class="language-javascript">// 示例：材料变更时自动更新密度
...
function onQuantityChanged(eventArgs) {
  ...
  eventArgs.item.setProperty('total_price', total);
    ...
    <h2>五、事件生命周期</h2>
    <pre v-pre><code class="language-text">数据改变
  │
  ▼
┌──────────────────┐
│  Validate 事件    │  ← 验证 + 格式转换
