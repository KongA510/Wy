<template>
  <article class="doc-content">
    ...
    <h3>3.1 创建菜单</h3>
    <pre v-pre><code class="language-javascript">// 创建菜单
const menu = document.createElement('aras-menu');
document.body.appendChild(menu);</code></pre>

    <h3>3.2 状态（state）</h3>
    <pre v-pre><code class="language-javascript">// state — 描述菜单当前状态的对象，不要直接操作，使用 setState 或 applyData
menu.state = {
  data: Map,             // 菜单数据
  ...
  focusedItems: string[]   // 当前获得焦点的菜单项
    ...
    <pre v-pre><code class="language-javascript">// 更新状态并触发重新渲染
menu.setState({
  ...
  focusedItems: ['edit_entry']
    ...
    <pre v-pre><code class="language-javascript">// 设置菜单数据并自动渲染
menu.applyData(menuData, ['new', 'open', 'save', 'sep_1', 'settings']);</code></pre>

    <h3>3.5 resetList()</h3>
    <pre v-pre><code class="language-javascript">// 清除展开和焦点状态
menu.resetList();</code></pre>

    <h3>3.6 calcSubmenuPosition(parentMenuItem, options)</h3>
    <pre v-pre><code class="language-javascript">// 计算子菜单的显示位置（避免溢出屏幕）
menu.calcSubmenuPosition(parentElement, {
  // 自动计算和调整位置
});</code></pre>

    <h2>四、菜单项数据结构</h2>
    <pre v-pre><code class="language-javascript">const menuData = new Map([
  ['new', {
    label: '新建',
    icon: '../images/new.svg',
    shortcut: 'Ctrl+N',
...
menu.applyData(menuData, roots);</code></pre>
    ...
    <pre v-pre><code class="language-javascript">// Dropdown 按钮 + 菜单实现
class DropdownMenu {
