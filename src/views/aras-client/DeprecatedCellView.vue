<template>
  <article class="doc-content">
    ...
    <h4>签名</h4>
    <pre v-pre><code class="language-javascript">Aras.Client.Controls.Public.Cell = function();</code></pre>
    <h4>参数</h4>
    <p><em>无参数信息</em></p>
    <h4>返回值</h4>
    ...
    <p>单元格的值，类型取决于列定义。</p>
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">// 获取 GridContainer 中指定行的单元格值
...
console.log(value);</code></pre>
    ...
    <p><em>无</em></p>
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">// 设置单元格值
...
function onEditCell(rowId, column, type, value, cell) {
  if (type === 0) {
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">var cell = grid.cells(rowId, "name");
var text = cell.getText();
top.aras.AlertInfo("当前选中行的名称: " + text);</code></pre>
    <hr />
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">// 遍历单元格获取行 ID
var cell = grid.cells("row_001", "item_number");
var rowId = cell.getRowId();
console.log("行ID: " + rowId);</code></pre>
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">var cell = grid.cells(rowId, columnName);
var colIndex = cell.getColumnIndex();
console.log("列索引: " + colIndex);</code></pre>
    ...
    <h4>样式操作综合示例</h4>
    <pre v-pre><code class="language-javascript">function onEditCell(rowId, column, type, value, cell) {
  // 根据值设置不同背景色
  if (column === "status") {
    switch (value) {
      case "Approved":
    ...
    <p><em>无信息提供</em></p>
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">// 检查并操作复选框单元格
...
if (cell.isCheckbox()) {
  ...
  if (!checked) {
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">// 编辑状态管理示例
function onEditCell(rowId, column, type, value, cell) {
  if (type === 0) {
    // 编辑开始 — 设置编辑类型和可编辑性
  ...
  if (type === 2) {
    ...
    <p><em>无</em></p>
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">// 动态设置下拉框编辑器
function onEditCell(rowId, column, type, value, cell) {
  if (type === 0) {
  ...
  if (type === 0 && column === "country") {
    ...
    <pre v-pre><code class="language-javascript">// 综合使用 Cell API 的 onEditCell 事件处理
function onEditCell(rowId, column, type, value, cell) {
  // type 说明：
  // 0 - 编辑开始前
  // 1 - 编辑进行中
