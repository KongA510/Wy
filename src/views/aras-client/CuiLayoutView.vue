<template>
  <article class="doc-content">
    ...
    <h2>三、生命周期</h2>
    <pre v-pre><code class="language-text">CUI Layout 初始化流程：
...
Step #N  .destroy() — 释放资源，销毁布局</code></pre>

    <h2>四、公共 API</h2>

    <h3>4.1 new CuiLayout(dom, location, options)</h3>
    <pre v-pre><code class="language-javascript">// 构造参数
const layout = new CuiLayout(
  ...
  'My_Custom_Location',                     // location: CUI Location 名称
  {
    ...
    <h3>4.2 .init()</h3>
    <pre v-pre><code class="language-javascript">// 默认逻辑包含：
// 1. 初始化 props 和 state
// 2. 获取 CUI 数据
// 3. 处理 CUI 数据
// 4. 启动控件
...
class MyCustomLayout extends CuiLayout {
  async init() {
    ...
    <pre v-pre><code class="language-javascript">class SearchViewLayout extends CuiLayout {
  // 在类级别定义状态的形状和初始值
  initialState = {
    searchText: '',
    searchResults: [],
    ...
    <pre v-pre><code class="language-javascript">// state 类似 React 组件的 state
// 用于在布局内的多个控件间共享数据
console.log(layout.state.searchText);        // 读取
console.log(layout.state.searchResults.length);

// 原则：只有多个控件都需要的数据才放入 state
// （类似 Redux 的"提升状态"理念）</code></pre>

    <h3>4.5 .setState(partialState)</h3>
    <pre v-pre><code class="language-javascript">// 更新部分状态，触发相关控件重新渲染
layout.setState({
  searchText: 'P-001',
  selectedIndex: 0
});
...
layout.on('state:changed', (newState, oldState) => {
  if (newState.searchText !== oldState.searchText) {
    ...
    <pre v-pre><code class="language-javascript">// 手动触发布局重新渲染
...
innefect.onItemChanged((item) => {
  ...
  layout.updateLayout();
    ...
    <h2>五、完整示例：搜索视图布局</h2>
    <pre v-pre><code class="language-javascript">// SearchViewCuiLayout.js
