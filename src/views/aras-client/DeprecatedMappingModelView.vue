<template>
  <article class="doc-content">
    ...
    <h4>签名</h4>
    <pre v-pre><code class="language-javascript">Aras.Modules.CMF.Public.MappingModel = function();</code></pre>
    <h4>参数</h4>
    <p><em>无参数信息</em></p>
    <h4>返回值</h4>
    <p><em>新创建的 MappingModel 实例</em></p>
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">// 推荐通过 Factory 创建 MappingModel 实例
var mappingModel = inArgs.factory.createMappingModel();</code></pre>
    <hr />

    <h3>.sourceId</h3>
    <p>获取或设置文档元素 ID。该属性标识需要绑定的文档中的目标元素。</p>
    <h4>签名</h4>
    <pre v-pre><code class="language-javascript">function get_sourceId();
function set_sourceId(value);</code></pre>
    <h4>参数</h4>
    <table>
      <thead>
        <tr>
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">var mappingModel = inArgs.factory.createMappingModel();
mappingModel.sourceId = 'Some Document Element Id';

// 获取当前设置的 sourceId
var currentSourceId = mappingModel.sourceId;
console.log("当前文档元素 ID: " + currentSourceId);</code></pre>
    <hr />

    <h3>.relatedId</h3>
    <p>获取或设置业务对象元素 ID。该属性标识文档元素需要绑定到的业务对象（如 Part）中的目标元素。</p>
    <h4>签名</h4>
    <pre v-pre><code class="language-javascript">function get_relatedId();
function set_relatedId(value);</code></pre>
    <h4>参数</h4>
    <table>
      <thead>
        <tr>
...
mappingModel.relatedId =
    'Some Business Object Element Id (for example Part Id)';</code></pre>
    <hr />

    <h3>.action</h3>
    <p>获取或设置绑定动作。用于指定映射关系的操作类型。</p>
    <h4>签名</h4>
    <pre v-pre><code class="language-javascript">function get_action();
function set_action(value);</code></pre>
    <h4>参数</h4>
    <table>
      <thead>
        <tr>
...
if (shouldUnbind) {
  ...
  mappingModel.action = 'Ignore';
    ...
    <h4>签名</h4>
    <pre v-pre><code class="language-javascript">function get_parentSourceId();
function set_parentSourceId(value);</code></pre>
    <h4>参数</h4>
    <table>
      <thead>
        <tr>
...
// 获取父元素 ID 用于层级关系构建
var parentId = mappingModel.parentSourceId;</code></pre>
    <hr />

    <h3>.sortOrder</h3>
    <p>获取或设置业务对象元素的排序顺序。用于控制映射结果中元素的排列顺序。</p>
    <h4>签名</h4>
    <pre v-pre><code class="language-javascript">function get_sortOrder();
function set_sortOrder(value);</code></pre>
    <h4>参数</h4>
    <table>
      <thead>
        <tr>
    ...
    <h4>示例</h4>
    <pre v-pre><code class="language-javascript">var mappingModel = inArgs.factory.createMappingModel();
mappingModel.sortOrder = 1;</code></pre>
    <hr />

    <h3>完整示例：构建文档到业务对象的映射关系</h3>
    <pre v-pre><code class="language-javascript">// 场景：将文档中的两个段落绑定到对应的 Part 属性
// 假设文档结构:
//   Document (root)
//     ├── Chapter1 (parent)
...
// 后续在 CMF 绑定流程中使用这些映射关系</code></pre>
