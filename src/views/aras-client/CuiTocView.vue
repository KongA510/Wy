<template>
  <article class="doc-content">
    ...
    <h2>二、公共 API</h2>
    <pre v-pre><code class="language-javascript">function cuiToc(
  ...
  destroy(): void              // 销毁 TOC 实例
    ...
    <pre v-pre><code class="language-javascript">// 单条 TOC 节点数据
{
  ...
  configId: "F1B2C3D4E5A6B7C8"                  // 配置标识
    ...
    <pre v-pre><code class="language-javascript">// 通过 configId 在 arasTocConfigs 中查找树配置
const tocConfig = {
  id: "F1B2C3D4E5A6B7C8",
  label: "设计文档分类",
  ...
  iconSrc: "../images/TOCItem.svg"
    ...
    <pre v-pre><code class="language-javascript">// CUI 数据 — TOC 控件定义
const tocControlData = {
  ...
  "sort_order": "0"
    ...
    <pre v-pre><code class="language-javascript">// cuiControls.js — TOC 控件注册
...
const controlRegistry = {
  ...
  'TocControl': {
    ...
    <pre v-pre><code class="language-javascript">// 自定义 TOC 编辑器 —— 分类导航面板
class ClassificationEditor {
  ...
  destroy() {
    ...
    <pre v-pre><code class="language-javascript">// 为 TOC 添加自定义节点或更新现有节点
...
function filterTocNodes(tocInstance, keyword) {
  ...
  tocInstance.updateItems(filtered);
    ...
    <h3>4.5 懒加载子节点</h3>
    <pre v-pre><code class="language-javascript">// TOC 懒加载 —— 点击展开时动态获取子节点
async function setupLazyLoading(tocElement) {
  tocElement.addEventListener('node-expand', async (e) => {
    ...
    <pre v-pre><code class="language-javascript">// BOM 结构浏览器 —— 工程师最常用的 TOC 场景
...
bomBrowser.init();</code></pre>

    <h2>五、客户端 Method（点击处理器）</h2>
    <p>通过 <code>client_method</code> 指定的点击处理函数，可以自定义节点点击后的导航行为：</p>

    <pre v-pre><code class="language-javascript">// 客户端 Method — 自定义 TOC 节点点击行为
