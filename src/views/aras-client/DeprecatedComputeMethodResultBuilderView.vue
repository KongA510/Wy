<template>
  <article class="doc-content">
    ...
    <h4>签名</h4>
    <pre v-pre><code class="language-javascript">Aras.Modules.CMF.Public.ComputeMethodResultBuilder = function();</code></pre>
    <h4>参数</h4>
    <p><em>无参数信息</em></p>
    <h4>返回值</h4>
    ...
    <pre v-pre><code class="language-javascript">// CMF 计算方法签名
function computeMethod(inArgs) {
  ...
  // ... 使用 resultBuilder 标记属性更新
    ...
    <h4>签名</h4>
    <pre v-pre><code class="language-javascript">markToUpdateStyle(propertyItemId: string, newStyle: Aras.Modules.CMF.Public.CmfStyle): void;</code></pre>
    <h4>参数</h4>
    <table>
      <thead>
        <tr>
...
if (cmfElements.length &gt; 0) {
    ...
    <h4>补充示例：条件样式更新</h4>
    <pre v-pre><code class="language-javascript">// 根据属性值动态设置样式
function computeMethod(inArgs) {
    ...
    <pre v-pre><code class="language-javascript">// 对多个属性项统一应用样式
function computeMethod(inArgs) {
    var elements = inArgs.factory.findDescendantElements('MyElement');
    if (elements.length &gt; 0) {
        var element = elements[0];
    ...
    <h4>签名</h4>
    <pre v-pre><code class="language-javascript">markToUpdateValue(propertyItemId: string, newValue: string): void;</code></pre>
    <h4>参数</h4>
    <table>
      <thead>
        <tr>
...
if (cmfElements.length &gt; 0) {
    ...
    <pre v-pre><code class="language-javascript">// 在计算方法中根据属性A计算属性B的值
function computeMethod(inArgs) {
    var elements = inArgs.factory.findDescendantElements('MyElement');
    if (elements.length &gt; 0) {
        var element = elements[0];
    ...
    <h4>补充示例：同时更新值和样式</h4>
    <pre v-pre><code class="language-javascript">// 同时更新属性的值和显示样式
function computeMethod(inArgs) {
    var elements = inArgs.factory.findDescendantElements('MyElement');
    if (elements.length &gt; 0) {
        var element = elements[0];
    ...
    <pre v-pre><code class="language-javascript">// 展示多次标记同一属性时最后一次生效的特性
function computeMethod(inArgs) {
    var elements = inArgs.factory.findDescendantElements('MyElement');
    if (elements.length &gt; 0) {
        var element = elements[0];
    ...
    <pre v-pre><code class="language-javascript">// 完整的 CMF 计算方法示例模板
