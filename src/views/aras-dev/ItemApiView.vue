<template>
  <article class="doc-content">
    ...
    <pre v-pre><code class="language-csharp">// 查询：设置条件
...
editPart.setAttribute("where", "Part.id='xxx'");</code></pre>
    ...
    <h2>四、关系操作</h2>
    <pre v-pre><code class="language-csharp">// 添加关系子项
...
for (var i = 0; i < rels.getItemCount(); i++) {
    ...
    <h2>五、生命周期操作</h2>
    <pre v-pre><code class="language-csharp">// 推进生命周期状态
var part = inn.newItem("Part", "get");
...
// 获取当前状态
var currentState = part.getProperty("state", "");</code></pre>
    ...
    <pre v-pre><code class="language-csharp">// 前端调用: inn.applyMethod("MyMethod", "&lt;myParam&gt;value&lt;/myParam&gt;")
// 后端接收:
var paramValue = this.getProperty("myParam", "");</code></pre>
