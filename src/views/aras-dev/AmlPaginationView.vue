...
&lt;/AML&gt;</code></pre>
    ...
    <p>IOM 调用：</p>
    <pre v-pre><code class="language-csharp">Item request = inn.newItem("Part", "get");
...
&lt;/AML&gt;</code></pre>
    ...
    <p>IOM 处理：</p>
    <pre v-pre><code class="language-csharp">// 查询1：获取总数
...
return this.getInnovator().newResult(
    ...
    <pre v-pre><code class="language-csharp">string aml = $@"
&lt;AML&gt;
  &lt;Item type='Part' action='get' select='id'&gt;
    &lt;name condition='like'&gt;*Bolt*&lt;/name&gt;
  &lt;/Item&gt;
...
int pageItemCount = pageResult.getItemCount();</code></pre>

    <h3>方案三：服务端方法封装（带完整分页信息返回）</h3>
    <p>将分页逻辑封装为一个可复用的服务端 Method，返回结构化结果：</p>

    <pre v-pre><code class="language-csharp">// 服务端 Method: pe_PaginateQuery
...
return response;</code></pre>
    ...
    <pre v-pre><code class="language-javascript">// 客户端 JS — 调用服务端分页方法
function onPaginateClick(page, pageSize) {
    ...
    <h3>REST API 分页调用</h3>
    <pre v-pre><code class="language-json">POST /InnovatorServer/Server/InnovatorServer.aspx
...
&lt;/AML&gt;</code></pre>
    ...
    <pre v-pre><code class="language-csharp">// =====================================================
...
return response;</code></pre>
    ...
    <pre v-pre><code class="language-xml">&lt;Item type="Part" action="get"
      page="1" pagesize="20"
      order_by="id ASC"
      select="id,name,item_number"&gt;
&lt;/Item&gt;</code></pre>
