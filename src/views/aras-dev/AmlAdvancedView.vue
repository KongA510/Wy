...
&lt;/AML&gt;</code></pre>
    ...
    <pre v-pre><code class="language-csharp">var result = inn.applyAML(aml);
// Parts: result.getItemByIndex(0)
// Documents: result.getItemByIndex(1)</code></pre>

    <h2>二、属性逻辑组合 (AND/OR)</h2>
    <pre v-pre><code class="language-xml">&lt;!-- AND 逻辑：多个条件子元素默认就是 AND 关系 --&gt;
...
&lt;/AML&gt;</code></pre>
    <p>IOM 处理关系：</p>
    <pre v-pre><code class="language-csharp">var parent = inn.newItem("Part", "get");
...
&lt;/Item&gt;</code></pre>
    ...
    <pre v-pre><code class="language-xml">&lt;Item type="Part BOM" action="get" select="quantity"&gt;
  &lt;related_id&gt;
    &lt;Item type="Part" action="get"
          select="id,name,item_number"&gt;
    &lt;/Item&gt;
