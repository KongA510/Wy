<template>
  <article class="doc-content">
    ...
    <pre v-pre><code class="language-csharp">/// &lt;summary&gt;
/// IC_DoSomething — 功能简述
/// &lt;/summary&gt;
...
finally
{
    ...
    <pre v-pre><code class="language-csharp">// ✅ 推荐：IOM API（类型安全、可读性好）
...
string aml = "&lt;AML&gt;&lt;Item type='Part' action='get'&gt;...";</code></pre>

    <h3>3.2 返回 JSON 而非 XML</h3>
    <pre v-pre><code class="language-csharp">// ✅ 推荐：序列化为 JSON
...
return resultItem;</code></pre>
    ...
    <pre v-pre><code class="language-csharp">// ✅ 推荐：使用 string.Format 或参数化
...
// 必须手动转义特殊字符</code></pre>
    ...
    <h2>六、日志规范</h2>
    <pre v-pre><code class="language-csharp">// 写入 Aras 服务器日志
Aras.Server.Core.Logging.LogError(
    "IC_DoSomething 执行失败: " + ex.Message);

// 或写入自定义日志（需要先创建日志配置文件）
// 推荐在关键操作（创建、删除、状态变更）时记录</code></pre>
