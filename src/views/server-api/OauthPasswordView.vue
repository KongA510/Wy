<template>
  <article class="doc-content">
    ...
    <h2>一、类定义</h2>
    <pre v-pre><code class="language-csharp">// PasswordTokenProvider — OAuth 2.0 密码模式 Token 提供者
...
public class PasswordTokenProvider : ITokenProvider
{
    ...
    <h2>二、密码模式流程</h2>
    <pre v-pre><code class="language-text">客户端                            认证服务器
  │                                  │
  │── POST /oauth2/token ───────────〉│
  │   grant_type=password             │
  │   username=admin                  │
...
class Program
{
    ...
    <pre v-pre><code class="language-csharp">// 检查 Token 是否过期
