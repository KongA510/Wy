...
// 生成：<and><state>Released</state><cost condition="gt">100</cost></and></code></pre>
...
// 生成：<or><state>Released</state><state>In Review</state></or></code></pre>
...
// 语义：state 不是 "Obsolete" 的 Part</code></pre>

    <h2>五、其他逻辑方法</h2>
    <pre v-pre><code class="language-csharp">// isLogical() — 判断 Item.node 是否为逻辑节点
...
public void removeLogical(Item logicalItem)</code></pre>

    <h2>六、嵌套逻辑示例</h2>
    <pre v-pre><code class="language-csharp">// 查询条件：(state="Released" AND cost>100) OR (state="In Review")
var query = inn.newItem("Part", "get");
