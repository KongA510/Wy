<template>
  <article class="doc-content">
    ...
    <h2>一、类定义</h2>
    <pre v-pre><code class="language-csharp">// JwtBearerClientAssertionProvider — JWT Bearer 断言提供者
public class JwtBearerClientAssertionProvider : IClientAssertionProvider
{
    ...
    <h2>二、相关类型</h2>
    <pre v-pre><code class="language-csharp">// 配置选项
...
// inn.Login(...) 内部会调用 assertionProvider.CreateClientAssertion()
    ...
    <h2>四、JWT 断言结构</h2>
    <pre v-pre><code class="language-csharp">// 生成的 JWT Payload（未签名前）类似：
// {
...
// }
//
// 签名算法：RS256（使用 X.509 证书的 RSA 私钥）</code></pre>
