...
// body: 传递给 Method 的 XML 字符串，每个子元素对应于 Method 中的一个 this.getProperty()</code></pre>
    ...
    <h2>二、调用流程</h2>
    <pre v-pre><code class="language-text">客户端 (Browser/JS)
...
服务端 (IIS / Aras Server)
  ...
  ... 业务逻辑 ...
  return inn.newResult(jsonString)</code></pre>

    <h2>三、Body 参数构造</h2>
    <pre v-pre><code class="language-csharp">// 服务端读取
string sender = this.getProperty("sender", "");
string data = this.getProperty("data", "");

// 对应前端 body 格式：
// <sender>admin</sender><data>hello</data></code></pre>
    ...
    <h2>四、.NET 客户端调用示例</h2>
    <pre v-pre><code class="language-csharp">// 独立 .NET 客户端调用
