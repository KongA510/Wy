<template>
  <article class="doc-content">
    ...
    <pre v-pre><code class="language-csharp">// 重载 1：按名称获取
...
string nameFr = item.getProperty("name", "", "fr");   // 法语版本</code></pre>

    <h2>二、setProperty — 设置属性值</h2>
    <pre v-pre><code class="language-csharp">// 重载 1：按名称设值
...
item.setProperty("cost", "123.45");</code></pre>

    <h2>三、removeProperty — 删除属性</h2>
    <pre v-pre><code class="language-csharp">// 重载 1：删除指定名称属性
...
item.removeProperty("name", "fr");  // 仅删除法语版本</code></pre>

    <h2>四、多语言属性处理</h2>
    <pre v-pre><code class="language-csharp">// 多语言属性在 AML 中表现为同一属性名 + 不同 xml:lang 属性
...
string descFr = item.getProperty("description", "", "fr"); // 法文</code></pre>

    <h2>五、getPropertyCondition / setPropertyCondition</h2>
    <pre v-pre><code class="language-csharp">// 获取属性的 'condition' 属性（用于 get 操作的查询条件）
...
var result = query.apply();</code></pre>
