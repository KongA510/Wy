<template>
  <article class="doc-content">
    ...
    <h2>一、文件上传</h2>
    <pre v-pre><code class="language-csharp">// attachPhysicalFile(String) — 附加物理文件
...
public void setFilePropertyViaStream(string propertyName, string fileName, Stream stream)</code></pre>

    <h2>二、文件下载</h2>
    <pre v-pre><code class="language-csharp">// fetchFileProperty(String, FetchFileMode) — 下载文件
...
item.checkout(@"C:\temp\checkout\");</code></pre>

    <h2>三、getFileName / setFileName（已废弃）</h2>
    <pre v-pre><code class="language-csharp">// [Obsolete] getFileName
// [Obsolete] setFileName
// 这些方法仅对 type="File" 的 Item 有意义
// 请使用 ItemType 属性操作代替</code></pre>
