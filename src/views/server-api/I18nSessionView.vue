<template>
  <article class="doc-content">
    ...
    <h2>二、获取会话上下文</h2>
    <pre v-pre><code class="language-csharp">// 通过 Innovator 获取
var inn = this.newInnovator();
var ctx = inn.getI18NSessionContext();

// 通过 Item 获取（间接）
var innovator = item.getInnovator();
var ctx2 = innovator.getI18NSessionContext();</code></pre>

    <h2>三、格式转换</h2>
    <pre v-pre><code class="language-csharp">// ConvertToNeutral — 将本地化格式转为 AML neutral 格式
// 用于构造 AML 请求时
...
// 在德语 locale 中可能输出 "1.234,56"</code></pre>
...
for (int i = 0; i < result.getItemCount(); i++) {
    ...
    <h2>六、多语言属性的处理</h2>
    <pre v-pre><code class="language-csharp">// 多语言字段在 AML 中通过 xml:lang 属性区分
// 会话语言为法语 (fr) 时：
...
item.setProperty("description", "中文文本", "zh");</code></pre>
